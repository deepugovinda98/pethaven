...
router.post("/add-pet", upload.array("photos", 5), async (req, res) => {
    try {
        ...
        if (!req.body.name || !req.body.species || !req.body.breed) {
        ...
        const newPet = new Pet({
            ...
            photos: req.files.length > 0 ? req.files.map(file => file.path) : [], // Save file paths in DB
...
// Route to fetch pet details by ID
